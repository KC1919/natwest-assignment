...
ssh -i $SSH_KEY ec2-user@$EC2_INSTANCE_IP 'sudo yum update -y && sudo amazon-linux-extras install nginx1.12 -y && sudo systemctl start nginx && sudo systemctl enable nginx'
